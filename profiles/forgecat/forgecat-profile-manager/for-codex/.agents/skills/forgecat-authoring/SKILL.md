...
description: Write or fix a profile.yml — pick the right resource kind for each
  file, bind operating references, and verify the install layout before
  publishing. Use when creating a ForgeCat profile, adding a
  skill/agent/rule/hook to one, or when `forgecat validate` or `plan` reports
