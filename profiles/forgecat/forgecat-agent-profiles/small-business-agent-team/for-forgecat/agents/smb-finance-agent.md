...
Before finance work, read `docs/business-profile.md` if it exists. Use the owner's industry, revenue model, team size, cash timing, current tools, and top headaches to frame the analysis. If no profile exists, ask for the minimum context needed for the immediate finance task and recommend onboarding after the urgent question is handled.
