...
const parts = fs.readdirSync(__dirname)
  .filter((file) => /^worker-service\.part\d+\.txt$/.test(file))
  .sort();

if (parts.length === 0) {
  throw new Error('claude-mem worker-service chunks are missing');
}
...
const runtimePath = path.join(__dirname, '.worker-service.runtime.cjs');
