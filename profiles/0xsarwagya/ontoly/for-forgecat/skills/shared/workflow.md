...
5. Query `SemanticContext` or `IntentExpansion` when the user uses natural feature language, then query `EvidencePack` for compact graph context and the capability that matches the task before reading files.
