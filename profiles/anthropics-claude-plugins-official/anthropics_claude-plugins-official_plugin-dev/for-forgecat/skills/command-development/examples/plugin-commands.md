...
!`node scripts/quality-check.js $1`
...
**Security scan:**
!`bash scripts/security-scan.sh $1`

**Performance analysis:**
!`bash scripts/perf-analyze.sh $1`

**Best practices check:**
!`bash scripts/best-practices.sh $1`
...
Template structure: @templates/api-documentation.md
...
!`bash scripts/pre-release-check.sh $1`

**Step 2 - Build artifacts:**
!`bash scripts/build-release.sh $1`

**Step 3 - Run test suite:**
!`bash scripts/run-tests.sh`

**Step 4 - Package release:**
!`bash scripts/package.sh $1`
...
Deployment configuration for $1: @config/$1-deploy.json
...
- Plugin's linting rules: config/lint-rules.json
...
Run plugin analyzer: !`node scripts/analyze.js $1`
...
Check build script exists: !`test -x scripts/build.sh && echo "EXISTS" || echo "MISSING"`

Verify configuration available: !`test -f config/$1.json && echo "FOUND" || echo "NOT_FOUND"`

If all validations pass:

**Configuration:** @config/$1.json

**Execute build:** !`bash scripts/build.sh $1 2>&1`

**Validation results:** !`bash scripts/validate-build.sh $1 2>&1`
...
Load environment configuration: @config/$1-checks.json

Determine check level: !`echo "$1" | grep -E "^prod$" && echo "FULL" || echo "BASIC"`

**For production environment:**
- Full test suite: !`bash scripts/test-full.sh`
- Security scan: !`bash scripts/security-scan.sh`
- Performance audit: !`bash scripts/perf-check.sh`
- Compliance check: !`bash scripts/compliance.sh`

**For non-production environments:**
- Basic tests: !`bash scripts/test-basic.sh`
- Quick lint: !`bash scripts/lint.sh`
...
```markdown
Check exists: !`test -f path/file && echo "YES" || echo "NO"`
```
...
1. **Using relative paths instead of :**
   ```markdown
   # Wrong
