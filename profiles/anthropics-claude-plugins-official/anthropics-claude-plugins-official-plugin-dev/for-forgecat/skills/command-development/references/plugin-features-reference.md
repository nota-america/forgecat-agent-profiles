...
`` is a special environment variable available in plugin commands that resolves to the absolute path of the plugin directory.
...
Run analysis: !`node scripts/analyze.js`

Read template: @templates/report.md
```
...
Lint results: !`node bin/lint.js $1`
...
Configuration: @config/deploy-config.json
...
Use this template: @templates/api-report.md
...
Step 1 - Prepare: !`bash scripts/prepare.sh $1`
Step 2 - Config: @config/$1.json
Step 3 - Execute: !`bin/execute $1`
...
2. **Validate file existence:**
   ...
   If config exists, load it: @config.json
...
Load configuration: @deploy-config.json
...
Template: @templates/component-docs.md
...
Environment config: @config/$1.json
...
!`mkdir -p cache && date > cache/last-run.txt`
...
- Config exists: !`test -f config.json && echo "✓" || echo "✗"`
- Scripts exist: !`test -d scripts && echo "✓" || echo "✗"`
- Tools available: !`test -x bin/analyze && echo "✓" || echo "✗"`
...
Try processing: !`node scripts/process.js $1 2>&1 || echo "ERROR: $?"`
...
1. **Use  for all plugin-internal paths**
