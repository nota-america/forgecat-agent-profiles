...
This directory is intentionally not removed by `uninstall.sh`. If you want to discard your prospect data, back it up first and then remove only that scoped directory:

```bash
python3 - <<'PYRM'
from pathlib import Path
import shutil
shutil.rmtree(Path.home()/'.geo-prospects', ignore_errors=True)
PYRM
```
