...
version: 2.3.0
description: AI(ChatGPT·Claude·Gemini 등)가 쓴 한글 텍스트를 "사람이 쓴 글처럼" 윤문해주는 오케스트레이터
  스킬. 번역투·영어 인용 과다·기계적 병렬·관용구·피동태 남용·접속사 남발·리듬 균일성·이모지/불릿 과다 등 10대 카테고리 70개 AI 티
  패턴을 탐지·분류해 내용은 한 글자도 건드리지 않고 문체·리듬·표현만 자연스러운 한국어로 재작성한다. shim의
  route_hint(light|standard|heavy)로 경로를 정해 잘 쓴 글은 1콜, 표준은 2콜, 중증·장문만 3+콜(진단→겨냥
  윤문→finalize)로 처리한다. 트리거 — "AI 티 없애줘", "AI 같은 글 자연스럽게", "GPT/ChatGPT 문체", "AI
  번역투 고쳐", "사람이 쓴 것처럼 윤문", "AI 윤문", "ChatGPT 티 제거", "한글 AI 탐지·윤문", "AI 글 사람처럼",
  "번역투 제거", "영어 인용 많은 글 윤문", "AI 글 티 안 나게", "휴머나이저", "humanize Korean", "AI
  detector bypass 한글". 후속 작업 — "특정 카테고리만 다시", "윤문 강도 조정", "장르 바꿔서", "이 문단만", "2차
  윤문" 도 모두 이 스킬. 단순 맞춤법·오탈자 교정은 직접 처리, 번역은 번역 스킬, 내용 추가·삭제를 동반한 재작성은 별도 집필 스킬.
---
...
4. 사전 처리 shim을 Bash로 1회 실행:
   ```
   python3 .forgecat/profiles/@forgecat/im-not-ai/runtime/forgecat/prepare_monolith_input.py --run-dir _workspace/{run_id} --genre {genre}
   ```
...
2. shim으로 진단을 monolith 입력 앞에 결합 (Bash — LLM 콜 아님):
   ```
   python3 .forgecat/profiles/@forgecat/im-not-ai/runtime/forgecat/prepare_monolith_input.py --run-dir _workspace/{run_id} --genre {genre} --diagnosis _workspace/{run_id}/02_diagnosis.md
   ```
...
1. shim으로 진단 결합 (Bash). **heavy에서만** `--chunk`를 함께 줄 수 있다:
   ```
   python3 .forgecat/profiles/@forgecat/im-not-ai/runtime/forgecat/prepare_monolith_input.py --run-dir _workspace/{run_id} --genre {genre} --diagnosis _workspace/{run_id}/02_diagnosis.md --chunk
   ```
...
4. **청크 병렬(shim이 실제로 쪼갠 경우만)**:
   ...
   - 재조립: `python3 .forgecat/profiles/@forgecat/im-not-ai/runtime/scripts/reassemble_chunks.py --run-dir _workspace/{run_id}` → `03_reassembled.md`(passthrough 원문 삽입 + 문자수 대사). 이걸 `final.md`로 삼는다.
...
```
python3 .forgecat/profiles/@forgecat/im-not-ai/runtime/scripts/verify_gates.py \
