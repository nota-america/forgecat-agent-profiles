...
Korean AI-text humanizer profile converted from `epoko77-ai/im-not-ai`.

This ForgeCat package preserves the Claude Code skill surface as the canonical cross-platform profile: `humanize-korean`, `humanize`, `humanize-redo`, the specialist agents, taxonomy/reference files, and deterministic Python helper scripts used by the pipeline.
...
- Source platform is `multi-host`: upstream contains Claude Code plugin/skills, OpenAI Codex skill material, and Gemini CLI extension material.
- ForgeCat runtime compatibility remains `Partial` for Claude Code, Cursor, and Codex until admin-local fresh install and runtime probes promote platforms to `Tested`.
- Platform-native manifests (`.claude-plugin/plugin.json`, `gemini-extension.json`, `GEMINI.md`), Gemini/Codex native command files, tests, release docs, and presentation assets are intentionally kept for the conversion history/source archive rather than shipped in `for-forgecat/`; they are not required by the runtime Claude skill surface and some are not ForgeCat-trackable resources.
- Upstream metadata has a known version mismatch: README/skills say v2.2.0, `.claude-plugin/plugin.json` says 2.1.0, and `gemini-extension.json` says 1.5.0. The ForgeCat profile is versionless; registry publish assigns the package version.
- The original-source README below is preserved for provenance. Its install commands and links to `assets/`, `INSTALL.md`, `.claude/`, `codex/`, `commands/`, or native plugin files are upstream-native references, not guaranteed ForgeCat-installed paths. Use the ForgeCat install command and converted paths above for this package.
