...
이 파일은 upstream `CLAUDE.md`를 ForgeCat 프로필 설치 레이아웃에 맞게 조정한 운영 지침이다. 원본 저장소의 Claude/Gemini/Codex 네이티브 매니페스트(`.claude-plugin/`, `gemini-extension.json`, `GEMINI.md`, `commands/`, `codex/`, `install.sh` 등)는 source archive에 보존되지만 ForgeCat core artifact에는 설치되지 않는다.

```
im-not-ai/for-forgecat/
├── AGENTS.md                      # 본 파일 — ForgeCat 운영 지침
├── README.md / LICENSE / profile.yml
├── skills/                        # 스킬 3종 (humanize-korean 오케스트레이터 + humanize·humanize-redo 진입)
│   ├── humanize/SKILL.md
│   ├── humanize-redo/SKILL.md
│   └── humanize-korean/
│       ├── SKILL.md               # route_hint 3경로 분기·shim 배선
│       └── references/
...
└── _workspace/                    # 런타임 산출물 (run_id별, gitignored; 실행 중 생성)
    └── {YYYY-MM-DD-NNN}/
        ├── 01_input.txt · 00_metrics.json · 01_input_with_metrics.txt
        ├── final.md
        ├── 01_chunk_{NN}… · 02_chunk_{NN}_rewritten.txt · 03_reassembled.md
        ├── 02_diagnosis.md
        └── final_pre_finalize.md · 09_finalize.json
```
