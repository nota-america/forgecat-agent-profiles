...
ForgeCat does not register these optional hooks automatically. Their scripts are installed with the `source-driven-development` skill under `.claude/skills/source-driven-development/scripts/`; use those paths instead of the source-repository `hooks/` paths when opting in.
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash \"${CLAUDE_PROJECT_DIR}/.claude/skills/source-driven-development/scripts/sdd-cache-pre.sh\"",
            "timeout": 10
          }
        ]
      }
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash \"${CLAUDE_PROJECT_DIR}/.claude/skills/source-driven-development/scripts/sdd-cache-post.sh\"",
            "async": true,
            "timeout": 10
          }
        ]
...
}' | bash .claude/skills/source-driven-development/scripts/sdd-cache-post.sh
...
}' | bash .claude/skills/source-driven-development/scripts/sdd-cache-pre.sh
echo "exit=$?"
```
...
echo '{"tool_input":{"url":"...", "prompt":"..."}}' | bash .claude/skills/source-driven-development/scripts/sdd-cache-pre.sh
