...
**Do not cache:** anything whose staleness is a correctness bug (balances, permissions, inventory at checkout), or per-user data under a key that does not identify the user. See `.forgecat/profiles/@forgecat/addyosmani_agent-skills/references/performance-checklist.md` for request coalescing, write strategies, negative caching, and the cache checklist.
...
For detailed performance checklists, optimization commands, and anti-pattern reference, see `.forgecat/profiles/@forgecat/addyosmani_agent-skills/references/performance-checklist.md`.
