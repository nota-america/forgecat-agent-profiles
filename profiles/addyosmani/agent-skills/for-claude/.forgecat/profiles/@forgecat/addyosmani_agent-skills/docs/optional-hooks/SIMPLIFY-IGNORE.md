...
ForgeCat does not register this optional hook automatically. Its script is installed with the `code-simplification` skill at `.claude/skills/code-simplification/scripts/simplify-ignore.sh`; use that path instead of the source-repository `hooks/` path when opting in.
...
```json
{
  "hooks": {
    "PreToolUse": [
      {
        "matcher": "Read",
        "hooks": [{ "type": "command", "command": "bash \"${CLAUDE_PROJECT_DIR}/.claude/skills/code-simplification/scripts/simplify-ignore.sh\"" }]
      }
    ],
    "PostToolUse": [
      {
...
```bash
echo '{}' | bash .claude/skills/code-simplification/scripts/simplify-ignore.sh
```
