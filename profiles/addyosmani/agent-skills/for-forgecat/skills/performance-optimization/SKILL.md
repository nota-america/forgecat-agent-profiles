...
**Do not cache:** anything whose staleness is a correctness bug (balances, permissions, inventory at checkout), or per-user data under a key that does not identify the user. See `{{ref:performance-checklist}}` for request coalescing, write strategies, negative caching, and the cache checklist.
...
For detailed performance checklists, optimization commands, and anti-pattern reference, see `{{ref:performance-checklist}}`.
