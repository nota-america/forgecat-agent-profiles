...
skills:
  - .claude/skills/slides-grab
  - .claude/skills/slides-grab-card-news
  - .claude/skills/slides-grab-design
  - .claude/skills/slides-grab-html
  - .claude/skills/slides-grab-export
  - .claude/skills/slides-grab-image
  - .claude/skills/slides-grab-plan
...
Use the canonical gate in `.forgecat/profiles/@forgecat/nomadamas_slides-grab/skills/slides-grab-design/references/design-gate.md`.
...
3. Produce two read-only review reports:
   ...
   Each `Proceed` report must use the CLI-enforced structure from `.forgecat/profiles/@forgecat/nomadamas_slides-grab/skills/slides-grab-design/references/design-gate.md`: role title, `VERDICT: PASS`, confidence, rendered PNG evidence filenames, current `slide-*.html: <sha256>` fingerprints, `Unresolved Critical: 0`, `Blocking findings: None`, findings table, and all required checks marked `PASS`.
