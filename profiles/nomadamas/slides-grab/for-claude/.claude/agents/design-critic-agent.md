...
Use the canonical gate in `.claude/skills/slides-grab-design/references/design-gate.md`.
...
3. Produce two read-only review reports:
   ...
   Each `Proceed` report must use the CLI-enforced structure from `.claude/skills/slides-grab-design/references/design-gate.md`: role title, `VERDICT: PASS`, confidence, rendered PNG evidence filenames, current `slide-*.html: <sha256>` fingerprints, `Unresolved Critical: 0`, `Blocking findings: None`, findings table, and all required checks marked `PASS`.
