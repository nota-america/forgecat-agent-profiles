...
2. Load the chosen style's full spec:
   - If `style` is a bundled id (e.g. `glassmorphism`), load from `{{ref:designStyles}}` — colors, fonts, layout, signature elements, and things to avoid.
...
- Use `slides-grab tldraw` plus `{{ref:templates}}/diagram-tldraw.html` when that gives a cleaner, more export-friendly result.
