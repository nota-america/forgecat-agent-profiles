...
9. For quantitative slides, use Chart.js as the default charting library, preferably starting from `{{ref:templates}}/chart.html`. Keep scripts in `slide-*.html`, disable animation, use stable canvas wrappers, and fix any `empty-canvas` validation error before review.
