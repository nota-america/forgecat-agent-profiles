...
- `.forgecat/profiles/@forgecat/nomadamas_slides-grab/src/design-styles-data.json` and `.forgecat/profiles/@forgecat/nomadamas_slides-grab/src/design-diversity-data.json` — full style specs (colors, fonts, layout, signature elements, things to avoid) for all 95 bundled styles; read these after the user picks a style to ground your design tokens
