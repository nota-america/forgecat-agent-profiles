...
## Color Usage Rules
- Pull every color from the approved style spec in `.forgecat/profiles/@forgecat/nomadamas_slides-grab/src/design-styles-data.json` or the user-provided brand tokens. Do not invent fresh standalone hex colors mid-slide.
...
- Start from `.forgecat/profiles/@forgecat/nomadamas_slides-grab/templates/chart.html` when a chart slide needs a proven structure.
