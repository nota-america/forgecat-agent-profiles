...
- Pull every color from the approved style spec in `.forgecat/profiles/@forgecat/nomadamas_slides-grab/src/design-styles-data.js` (or the user's brand tokens when they override the bundled style). Do not invent fresh standalone hex colors mid-slide.
