...
> 1. `deploy-result.json` in session folder — if missing, read [`deploy-schemas.ts`](../deploy/references/deploy-schemas.ts.txt) and write it NOW with status, endpoints, health, `orphanedResourceGroups[]`
> 2. Portal deployment link printed in chat — if missing, generate from `$resId` pattern (see deploy/phase.md Step 6) and print now
