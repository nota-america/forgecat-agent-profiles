...
| 2. Update plan | `prepare-plan.json`: `services[]`, `naming.resources[]`, `costEstimate`, `deployStrategy` per [`prepare-schemas.ts`](../../prepare/references/prepare-schemas.ts.txt) |
...
| 5. Log | [`ScaffoldHealingAttempt`](scaffold-schemas.ts.txt) with `planLevelChange: true`. Counts toward healing cap |
