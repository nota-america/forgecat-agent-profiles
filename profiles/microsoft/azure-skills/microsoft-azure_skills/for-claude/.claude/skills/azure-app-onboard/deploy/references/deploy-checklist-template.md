...
## ⛔ Read deploy/phase.md
- You MUST `view` deploy/phase.md BEFORE running any `az deployment` command
- Path: `deploy/phase.md` (relative to the installed azure-app-onboard skill directory)
...
- ⛔ Read [`deploy-schemas.ts.txt`](deploy-schemas.ts.txt) for exact DeployResult field names
