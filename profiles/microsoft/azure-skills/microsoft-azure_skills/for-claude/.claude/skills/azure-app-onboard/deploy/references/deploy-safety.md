...
Created by scaffold validate sub-agent. If missing at deploy Step 5b, create from [`deploy-schemas.ts.txt`](deploy-schemas.ts.txt) with `status: "in-progress"`. Append to `deploymentNames[]` on each healing retry.
