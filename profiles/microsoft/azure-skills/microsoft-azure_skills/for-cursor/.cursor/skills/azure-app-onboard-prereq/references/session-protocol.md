...
>    - **Pointer exists** →
>      1. ⛔ **Read [`session-schemas.ts.txt`](session-schemas.ts.txt)** to get the exact field names and types for `AppOnboardContext`, `PrereqOutput`, and `PreparePlan`. Do not guess field names. Then read the pointed-to session's `context.json`. Display: "Found session from [lastModifiedUtc] — {statusSummary}."
