...
1. **`prereq-output.json`** — ⛔ Read [`prereq-schemas.ts.txt`](prereq-schemas.ts.txt) for `PrereqOutput` interface.
   ...
   >
   > ⛔ **`postDeployRecommendations[]`:** For each ⚠️ WARN, write per `PostDeployRecommendation` schema from [`session-schemas.ts.txt`](session-schemas.ts.txt): `{ "title", "reason", "effort": "low|medium|high", "services": [] }`.
