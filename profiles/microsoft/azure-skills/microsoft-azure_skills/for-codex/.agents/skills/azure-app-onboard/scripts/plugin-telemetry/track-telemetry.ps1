...
$skillsDir = if ($env:AZURE_SKILLS_PLUGIN_ROOT_SKILLS_DIR) { $env:AZURE_SKILLS_PLUGIN_ROOT_SKILLS_DIR } else { Join-Path (Split-Path -Parent (Split-Path -Parent $scriptDir)) 'skills' }
...
function Get-PluginVersion {
    $pluginManifestPath = if ($env:AZURE_SKILLS_PLUGIN_ROOT_MANIFEST) { $env:AZURE_SKILLS_PLUGIN_ROOT_MANIFEST } else { Join-Path (Split-Path -Parent $skillsDir) '.plugin/plugin.json' }
