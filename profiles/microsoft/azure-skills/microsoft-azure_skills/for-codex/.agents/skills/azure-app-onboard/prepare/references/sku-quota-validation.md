...
> Return it in the quota output's per-service `version` field; the orchestrator copies it to `prepare-plan.json.services[].version` at plan-write (exact patch required — see [prepare-schemas.ts](prepare-schemas.ts.txt) `version`). Record the bump in `assumptions[]` if the detected version was upgraded.
