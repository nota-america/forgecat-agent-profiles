...
description: Build students' capacity to interrogate a historical source before
  reading — asking who authored it, when, why, and what this means for
  reliability. Use when students read documents without attending to authorship.
disable-model-invocation: false
user-invocable: true
effort: medium
skill_id: sourcing-skill-builder
...
evidence_sources:
  - "Wineburg (1991) — Historical problem solving: cognitive processes in the
    evaluation of documentary and pictorial evidence"
  - "Wineburg (1998) — Reading Abraham Lincoln: an expert/expert study in
    historical cognition"
  - "Wineburg (2007) — Unnatural and essential: the nature of historical
    thinking"
  - "Gottlieb & Wineburg (2012) — Between Veritas and Communitas: epistemic
    switching in the reading of academic and sacred history"
...
input_schema:
  required:
    ...
    - field: student_level
      type: string
      description: Age/year group and current level of experience with primary source
        analysis
    - field: current_challenge
      type: string
      description: What students currently do or fail to do when encountering a
        historical source — the observable behaviour that signals sourcing is
        not yet habitual
  optional:
    - field: source_type
      type: string
      description: The type of historical source students are working with — e.g.
        written testimony, government document, photograph, political cartoon,
        newspaper article, map, personal letter
    - field: available_documents
      type: string
      description: Description of the specific documents students will encounter in
        the lesson or unit
    - field: curriculum_framework
      type: string
      description: "From context engine: relevant curriculum standards or historical
        thinking framework in use — e.g. DIG, Seixas Big Six, national
        curriculum requirements"
    - field: prior_instruction
      type: string
      description: What sourcing instruction students have already received — so
        recommendations build on rather than repeat previous work
output_schema:
  type: object
  fields:
    - field: sourcing_progression
      type: object
      description: What sourcing looks like at novice, developing, and proficient
        levels for this student age group — with observable indicators at each
        level, distinguishing between mentioning a source and interrogating it
    - field: explicit_instruction_sequence
      type: object
      description: A three-part sequence (model → guided practice → independent
        practice) for teaching sourcing with the specified source type,
        including a teacher think-aloud script demonstrating what an expert
        reader does with bibliographic information before reading the text
    - field: guiding_questions
      type: array
      description: Source-type-specific questions students should ask before and
        during reading — adapted from the DIG sourcing protocol but calibrated
        to the developmental level and source type
    - field: common_failure_patterns
      type: array
      description: Specific ways students at this level typically fail to source —
        with targeted instructional responses for each
    - field: assessment_indicators
      type: object
      description: Observable signs that students are developing sourcing as a
        habitual practice rather than a prompted behaviour — what to listen for
        in discussion, what to look for in written work
chains_well_with:
