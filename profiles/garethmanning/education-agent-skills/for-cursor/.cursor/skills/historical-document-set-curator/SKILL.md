...
description: Design a document set for a document-based lesson — selecting and
  sequencing sources for analytical tension around a central question. Use when
  assembling sources for a new lesson or when an existing set produces flat
  responses.
disable-model-invocation: false
user-invocable: true
effort: high
skill_id: historical-document-set-curator
...
evidence_sources:
  ...
  - "Wineburg (2007) — Unnatural and essential: the nature of historical
    thinking"
  - "Caswell (2014) — Seeing yourself in history: community archives and the
    fight against symbolic annihilation"
  - Theimer (2012) — Archives in context and as context
input_schema:
  required:
    - field: central_question
      type: string
      description: The central historical question the document set must support — the
        inquiry that students will investigate using these sources
    - field: student_level
      type: string
      description: Age/year group, reading level, and experience with document-based inquiry
    - field: target_skills
      type: string
      description: Which historical thinking skills the document set should foreground
        — e.g. sourcing and corroboration, close reading, all four
  optional:
    - field: existing_sources
      type: string
      description: Sources the teacher already has or plans to use — the skill will
        evaluate these and suggest additions, removals, or resequencing
    - field: set_size
      type: string
      description: Target number of documents — typically 2–5 for a single lesson,
        more for a multi-day unit
    - field: source_constraints
      type: string
      description: Practical constraints — e.g. sources must be freely available
        online, must be in English, must be shorter than 200 words each, must
        include visual sources
    - field: representation_priorities
      type: string
      description: Whose perspectives should be represented in the set — e.g. ensure
        Indigenous voices are included, include perspectives from multiple
        social classes, include women's perspectives
...
output_schema:
  ...
  fields:
    - field: document_set_design
      type: object
      description: The recommended document set with rationale — which sources, in
        what sequence, and why each earns its place
    - field: analytical_tensions
      type: array
      description: The specific tensions, contradictions, or complementarities built
        into the set — what students will discover when they compare sources
    - field: skill_affordances
      type: object
      description: For each target skill, which documents in the set provide the best
        opportunity to practise it and why
    - field: sequencing_rationale
      type: string
      description: Why the documents are ordered as they are — what each round of
        reading adds to the investigation
    - field: representation_audit
      type: object
      description: Whose perspectives are present and absent in the set, with honest
        acknowledgement of gaps and suggestions for addressing them
chains_well_with:
