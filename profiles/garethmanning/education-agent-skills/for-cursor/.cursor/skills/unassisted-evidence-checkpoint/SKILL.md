...
description: After scaffolded practice, run an unassisted check — a problem with
  no AI help. Separates what the learner can do with support from what they can
  do independently. Critical for preventing phantom attainment.
disable-model-invocation: false
user-invocable: true
effort: light
skill_id: unassisted-evidence-checkpoint
...
evidence_sources:
  - Bastani et al. (2025) — Generative AI can harm learning (PNAS) — unguarded
    GPT-4 reduced unassisted performance by 17%
  - Koedinger & Aleven (2007) — Exploring the assistance dilemma in experiments
    with cognitive tutors
  - Kapur & Rummel (2012) — Productive failure and the nature of prior knowledge
    for learning
