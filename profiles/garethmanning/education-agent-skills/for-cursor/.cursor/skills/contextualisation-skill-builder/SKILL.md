...
description: Build students' capacity to place historical documents in their
  temporal and social context. Use when students read sources without
  considering what was happening at the time, or know the context but don't
  deploy it.
disable-model-invocation: false
user-invocable: true
effort: medium
skill_id: contextualisation-skill-builder
...
input_schema:
  required:
    ...
    - field: current_challenge
      type: string
      description: What students currently do or fail to do when contextualising — the
        observable behaviour that signals contextualisation is not yet developed
  optional:
    - field: source_type
      type: string
    ...
    - field: background_knowledge
      type: string
      description: What background knowledge students already possess about the period
        — critical because contextualisation requires knowledge to deploy, and
        the gap may be knowledge rather than skill
...
output_schema:
  ...
  fields:
    - field: contextualisation_progression
      type: object
      description: What contextualisation looks like at novice, developing, and
        proficient levels — with observable indicators distinguishing between
        knowing background information and deploying it to transform
        interpretation of a document
    - field: explicit_instruction_sequence
      type: object
      description: A three-part sequence for teaching contextualisation, designed for
        the greater complexity of this skill compared to sourcing and close
        reading — includes strategies for making the invisible mental move of
        connecting a document to its historical moment visible and practicable
    - field: contextualisation_prompts
      type: array
      description: Prompts that guide students to connect a document to its historical
        moment — calibrated to developmental level and source type
    - field: common_failure_patterns
      type: array
      description: Specific ways students fail to contextualise, including the
        critical distinction between possessing background knowledge and
        activating it during document reading
    - field: assessment_indicators
      type: object
      description: Observable signs that students are contextualising — what to listen
