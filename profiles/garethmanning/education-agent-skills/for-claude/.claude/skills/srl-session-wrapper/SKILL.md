...
description: Wrap a learning session in a plan → monitor → reflect cycle. Use at
  the start of any substantial study session to set goals, mid-session to check
  strategy, and at session end to consolidate what changed. Builds
  self-regulated learning as a habit.
disable-model-invocation: false
user-invocable: true
effort: light
skill_id: srl-session-wrapper
...
evidence_sources:
  - "Zimmerman (2000) — Attaining self-regulation: a social cognitive
    perspective"
  - Bannert (2007) — Metakognition beim Lernen mit Hypermedien (metacognitive
    prompting in hypermedia learning)
  ...
  - Frontiers in Education meta-analysis (2025) — AI-supported self-regulated
    learning
input_schema:
  required:
    ...
    - field: session_type
      type: string
      description: opening | mid-session | closing | full-wrap — which phase of the
        wrapper to run
  optional:
    - field: session_goal_from_opening
      type: string
      description: The goal set in the opening phase — required for mid-session and
        closing phases
