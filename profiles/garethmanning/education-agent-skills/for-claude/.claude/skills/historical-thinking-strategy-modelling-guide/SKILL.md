...
description: Design a teacher think-aloud that models historical thinking
  strategies with a specific document. Use when planning explicit strategy
  instruction or when students follow a protocol without understanding the
  underlying reasoning.
disable-model-invocation: false
user-invocable: true
effort: medium
skill_id: historical-thinking-strategy-modelling-guide
...
evidence_sources:
  ...
  - "Rosenshine (2012) — Principles of instruction: research-based strategies
    that all teachers should know"
input_schema:
  required:
    - field: document_description
      type: string
      description: The specific historical document the teacher will model with —
        including its source information (author, date, publication context) and
        a summary or key passage of its content
    - field: target_strategies
      type: string
      description: "Which historical thinking strategies to model — one or more of:
        sourcing, close reading, contextualisation, corroboration. If
        corroboration, a second document must be described."
    - field: student_level
      type: string
  ...
  optional:
    - field: current_challenge
      type: string
      description: What students currently struggle with — allows the think-aloud to
        specifically target the gap between what students do and what expert
        readers do
    - field: second_document
      type: string
      description: A second document to compare with the first. REQUIRED if
        corroboration is listed in target_strategies — the modelling cannot
        proceed without it.
    - field: lesson_context
      type: string
      description: Where in the lesson this modelling will occur — e.g. opening a new
        document set, transitioning from guided to independent practice,
        demonstrating a strategy students have not yet encountered
...
output_schema:
  ...
  fields:
    - field: think_aloud_script
      type: string
      description: A complete teacher think-aloud script that models the target
        strategies with the specified document — including pauses,
        self-questions, hypotheses, and the specification of ignorance
    - field: cognitive_moves_annotated
      type: array
      description: The think-aloud script annotated with labels identifying each
        cognitive move — so the teacher understands what they are modelling and
        can name it for students
    - field: common_pitfalls
      type: array
      description: Common mistakes teachers make when modelling historical thinking —
        e.g. narrating the answer rather than the thinking process, modelling
        too quickly, not pausing to show uncertainty
    - field: transition_to_practice
      type: object
      description: How to move from the modelled think-aloud to guided student
        practice — what to hand over, what to scaffold, what to watch for
chains_well_with:
