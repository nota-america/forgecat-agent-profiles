...
_DISABLED_REVIEW_MODE=$("$GSTACK_ROOT/bin/gstack-config" get codex_reviews 2>/dev/null) || {
  echo 'Cannot read codex_reviews; disabled outside coverage was not recorded.' >&2
  exit 1
}
if [ "$_DISABLED_REVIEW_MODE" = disabled ]; then
  "$GSTACK_ROOT/bin/gstack-review-log" '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"skipped","source":"none","host":"claude","outside_provider":"codex","outside_status":"disabled","phase":"plan-review","commit":"'"$(git rev-parse --short HEAD 2>/dev/null || true)"'"}'
fi
```
...
**Persist the result:**
```bash
$GSTACK_ROOT/bin/gstack-review-log '{"skill":"codex-plan-review","timestamp":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","status":"STATUS","source":"SOURCE","host":"claude","outside_provider":"codex","outside_status":"OUTSIDE_STATUS","phase":"plan-review","commit":"'"$(git rev-parse --short HEAD)"'"}'
```
...
1. Brain trust policy for the active endpoint is `personal` (check via
   `$GSTACK_ROOT/bin/gstack-config get brain_trust_policy@<endpoint-hash>`).
...
eval "$($GSTACK_ROOT/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
  $GSTACK_ROOT/bin/gstack-brain-cache invalidate developer-persona --project "$SLUG" 2>/dev/null || true
```
...
```bash
eval "$($GSTACK_ROOT/bin/gstack-slug 2>/dev/null)" 2>/dev/null || true
($GSTACK_ROOT/bin/gstack-brain-cache refresh --project "$SLUG" 2>/dev/null &) || true
```
