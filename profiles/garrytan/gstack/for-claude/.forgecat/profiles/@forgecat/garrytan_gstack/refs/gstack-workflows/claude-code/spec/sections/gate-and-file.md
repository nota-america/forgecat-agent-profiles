...
REDACT_BODY_EOF
if REDACT_JSON=$("$GSTACK_ROOT/bin/gstack-redact" --from-file "$REDACT_FILE" --repo-visibility "$REDACT_VIS" --self-email "$(git config user.email 2>/dev/null)" --json); then REDACT_CODE=0; else REDACT_CODE=$?; fi
case "$REDACT_CODE" in
...
reuse it; write the exact bytes to `$REDACT_FILE`; `$GSTACK_ROOT/bin/gstack-redact --from-file "$REDACT_FILE"
