...
if [ -d "$_PROJ" ]; then
  ...
  if [ -f "$_PROJ/decisions.active.json" ]; then
    ...
    $GSTACK_ROOT/bin/gstack-decision-search --recent 5 2>/dev/null
    echo "--- END DECISIONS ---"
  fi
  echo "--- END ARTIFACTS ---"
fi
...
**Cross-session decisions.** Honor listed `ACTIVE DECISIONS` and their rationale; do not silently re-litigate them, and announce planned reversals. Use `$GSTACK_ROOT/bin/gstack-decision-search` for past-decision questions. Log DURABLE decisions by you or the user (architecture, scope, tool/vendor choice, reversal; not trivial or turn-level choices) with `$GSTACK_ROOT/bin/gstack-decision-log` (`--supersede <id>` for reversals). Reliable and local; gbrain not required.
...
```bash
$GSTACK_ROOT/bin/gstack-question-log '{"skill":"autoplan","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"SESSION_ID"}' 2>/dev/null || true
```
...
```bash
$GSTACK_ROOT/bin/gstack-skill-end --skill "autoplan" --outcome OUTCOME \
...
bun -e 'console.log(require("fs").realpathSync(process.argv[1]))' "$GSTACK_ROOT/bin/gstack-autoplan-snapshot.ts"
```
...
- Phase 1: `$GSTACK_ROOT/plan-ceo-review/SKILL.md`
- Phase 2: `$GSTACK_ROOT/plan-design-review/SKILL.md` (only if UI scope detected)
- Phase 2.5: `$GSTACK_ROOT/plan-devex-review/SKILL.md` (only if DX scope detected)
- Phase 3: `$GSTACK_ROOT/plan-eng-review/SKILL.md`
...
> **STOP.** Before starting Phase 1 (CEO review — always runs, after the Phase 0.5 preflight), Read `$GSTACK_ROOT/autoplan/sections/ceo-phase.md` and execute it
...
> **STOP.** Before starting Phase 2 (design review — ONLY if UI scope was detected in Phase 0; skip the read entirely otherwise), Read `$GSTACK_ROOT/autoplan/sections/design-phase.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
> **STOP.** Before starting Phase 2.5 (DX review — ONLY if developer-facing scope was detected in Phase 0; skip the read entirely otherwise), Read `$GSTACK_ROOT/autoplan/sections/dx-phase.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
> **STOP.** Before starting Phase 3 (eng review — always runs, after the Pre-Phase 3 checklist), Read `$GSTACK_ROOT/autoplan/sections/eng-phase.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
> **STOP.** Before presenting the Final Approval Gate (Phase 4) — the aggregator computes $AGGREGATED_TASKS that the gate message substitutes, Read `$GSTACK_ROOT/autoplan/sections/tasks-aggregator.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
```bash
$GSTACK_ROOT/bin/gstack-review-log '{"skill":"autoplan-voices","run_id":"AUTOPLAN_RUN_ID","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","host":"claude","outside_provider":"codex","outside_status":"OUTSIDE_STATUS","phase":"ceo","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'

$GSTACK_ROOT/bin/gstack-review-log '{"skill":"autoplan-voices","run_id":"AUTOPLAN_RUN_ID","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","host":"claude","outside_provider":"codex","outside_status":"OUTSIDE_STATUS","phase":"eng","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
...
```bash
$GSTACK_ROOT/bin/gstack-review-log '{"skill":"autoplan-voices","run_id":"AUTOPLAN_RUN_ID","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","host":"claude","outside_provider":"codex","outside_status":"OUTSIDE_STATUS","phase":"design","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
...
```bash
$GSTACK_ROOT/bin/gstack-review-log '{"skill":"autoplan-voices","run_id":"AUTOPLAN_RUN_ID","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","host":"claude","outside_provider":"codex","outside_status":"OUTSIDE_STATUS","phase":"dx","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
