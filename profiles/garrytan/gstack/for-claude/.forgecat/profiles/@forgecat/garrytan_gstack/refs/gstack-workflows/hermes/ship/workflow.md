...
"$_SS" --skill "ship" --model "claude" --parent-pid "$PPID" --brain-health \
...
**Dispatch this step as a subagent** using delegate_task with `a fresh native leaf worker`. The subagent runs the coverage audit in a fresh context window — the parent only sees the conclusion, not intermediate file reads. This is context-rot defense.
...
**Dispatch this step as a subagent** using delegate_task with `a fresh native leaf worker`. The subagent reads the plan file and every referenced code file in its own fresh context. Parent gets only the conclusion.
...
```bash
bun --no-env-file run $GSTACK_BIN/gstack-design-detect.ts probe --host hermes
```

On `IMPECCABLE_READY`, scan the changed frontend files (the wrapper derives them from git; hook presence does not skip this):

```bash
_DJ=$(mktemp); bun --no-env-file run $GSTACK_BIN/gstack-design-detect.ts scan --changed <base> --format gstack --host hermes > "$_DJ"; echo "DETECT_EXIT_CODE=$?"; echo "DETECT_JSON=$_DJ"
```
...
```bash
$GSTACK_BIN/gstack-review-log '{"skill":"design-review-lite","host":"hermes","outside_provider":"codex","outside_status":"OUTSIDE_STATUS","phase":"design-lite","timestamp":"TIMESTAMP","status":"STATUS","findings":N,"auto_fixed":M,"detector":D,"commit":"COMMIT","completed":COMPLETED,"converged":CONVERGED}' --finish DESIGN_START
```
...
**Dispatch the fetch + classification as a subagent** using delegate_task with `a fresh native leaf worker`. The subagent pulls every Greptile comment, runs the escalation detection algorithm, and classifies each comment. Parent receives a structured list and handles user interaction + file edits.
...
**Dispatch /document-release as a subagent** using delegate_task — never the Skill tool, even though document-release appears in your skills list — with `a fresh native leaf worker`. The subagent gets a fresh context window — zero rot from the preceding 17 steps. It also runs the **full** `/document-release` workflow (with CHANGELOG clobber protection, doc exclusions, risky-change gates, named staging, race-safe PR body editing) rather than a weaker reimplementation. The dispatch prompt marks the subagent session as spawned (`GSTACK_SESSION_KIND=spawned`) so document-release's interactive gates auto-choose their recommended options instead of prose-stopping — a prose-STOP inside the subagent breaks the parent's LAST-line JSON parse and drops the Documentation section (#2733).
