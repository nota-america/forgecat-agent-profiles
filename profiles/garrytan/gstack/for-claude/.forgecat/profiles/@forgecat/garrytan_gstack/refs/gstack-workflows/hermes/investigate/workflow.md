...
"$_SS" --skill "investigate" --model "claude" --parent-pid "$PPID" --brain-health \
...
Call `gstack_control` with `operation: "activate"`, `mode: "freeze"`, and the resolved absolute affected-directory `boundary`. Do not claim a lock before confirmation.
