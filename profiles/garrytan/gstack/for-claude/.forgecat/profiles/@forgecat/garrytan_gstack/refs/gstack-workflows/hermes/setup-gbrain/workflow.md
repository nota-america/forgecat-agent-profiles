...
"$_SS" --skill "setup-gbrain" --model "claude" --parent-pid "$PPID" --brain-health \
...
if ! gbrain init --pglite --json "$@"; then
  ...
  echo "PGLite directory at ~/.gbrain/pglite/ may be in a partial state — \`move the PGLite directory to a recovery location after stopping its processes\` if needed before retrying." >&2
  exit 1
fi
...
if ! gbrain init --pglite --json "$@"; then
  ...
  echo "gbrain init failed. Existing config (if any) was restored. PGLite at ~/.gbrain/pglite/ may be in a partial state — \`move the PGLite directory to a recovery location after stopping its processes\` to reset." >&2
