...
```bash
$GSTACK_ROOT/bin/gstack-question-log '{"skill":"setup-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"SESSION_ID"}' 2>/dev/null || true
```
...
```bash
$GSTACK_ROOT/bin/gstack-skill-end --skill "setup-gbrain" --outcome OUTCOME \
...
> **STOP.** Before running the Step 1.5 broken-engine remediation — Step 1's detect returned `gbrain_local_status` of `broken-db` or `broken-config` and no shortcut flag was passed, Read `$GSTACK_ROOT/setup-gbrain/sections/engine-remediation.md` and execute it
...
> **STOP.** Before initializing the brain in Step 4 — run ONLY the procedure for the path picked in Step 2 (Paths 1/2a/2b/3/4 or Switch; also holds the PAT scope disclosure that `--cleanup-orphans` re-uses), Read `$GSTACK_ROOT/setup-gbrain/sections/brain-init.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
> **STOP.** Before running the Step 7.5 transcript & memory ingest gate on Paths 1, 2a, 2b, or 3 (Path 4 skips this section entirely — see the skeleton's skip note), Read `$GSTACK_ROOT/setup-gbrain/sections/transcript-gate.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
> **STOP.** Before persisting the Step 8 `## GBrain Configuration` block to CLAUDE.md (and the Search Guidance block after Step 9 passes), Read `$GSTACK_ROOT/setup-gbrain/sections/claude-md-persist.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
- **Concurrent-run lock.** At skill start, `mkdir ~/.gstack/.setup-gbrain.lock.d`
  ...
  is running. Wait for it, or `move the stale lock directory to a recovery location after checking no setup process is running` if
