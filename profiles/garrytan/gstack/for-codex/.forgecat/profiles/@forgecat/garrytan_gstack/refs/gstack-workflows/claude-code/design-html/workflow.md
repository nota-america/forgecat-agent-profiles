...
```bash
$GSTACK_ROOT/bin/gstack-question-log '{"skill":"design-html","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"SESSION_ID"}' 2>/dev/null || true
```
...
```bash
$GSTACK_ROOT/bin/gstack-skill-end --skill "design-html" --outcome OUTCOME \
...
> **STOP.** Before analyzing the design or making any layout/visual decision (Step 1 onward) — the UX-principles doctrine governs every design choice, Read `$GSTACK_ROOT/design-html/sections/doctrine.md` and execute it
...
> **STOP.** Before writing the finalized HTML in Step 3 — the Pretext wiring patterns and API cheatsheet are the required reference for all text-layout code, Read `$GSTACK_ROOT/design-html/sections/pretext-patterns.md` and execute it
> in full. Do not work from memory — that section is the source of truth for this step.
...
_PRETEXT_VENDOR=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$_ROOT/$GSTACK_ROOT/design-html/vendor/pretext.js" ] && _PRETEXT_VENDOR="$_ROOT/$GSTACK_ROOT/design-html/vendor/pretext.js"
...
```bash
_DJ=$(mktemp); bun --no-env-file run $GSTACK_ROOT/bin/gstack-design-detect.ts scan --format gstack --host claude <finalized.html> > "$_DJ"; echo "DETECT_EXIT_CODE=$?"; echo "DETECT_JSON=$_DJ"
```
