...
#
#   git commit -m "wip" && recursive-delete ROOT-OR-HOME/   ->  CMD='git commit -m \'   -> allowed
#   bash -c "recursive-delete ROOT-OR-HOME/"                ->  CMD='bash -c \'         -> allowed
#   echo "x"; recursive-delete ROOT-OR-HOME/                ->  CMD='echo \'            -> allowed
#
...
if [ "$_IS_SIMPLE" -eq 1 ]; then
  ...
  if printf '%s' "$CMD" | grep -qE '^[[:space:]]*(sudo[[:space:]]+)?rm[[:space:]]' 2>/dev/null \
    ...
    set -f
    for _TOK in $CMD; do
      # Strip one layer of surrounding quotes: recursive-delete ROOT-OR-HOME/" is still recursive-delete ROOT-OR-HOME/.
...
#   recursive-delete ROOT-OR-HOME/ # rm -rf node_modules
...
#     shape against EACH line, so `recursive-delete ROOT-OR-HOME/\nrm -rf node_modules` would be
...
# recursive-delete ROOT-OR-HOME/ recursive-delete ROOT-OR-HOME/ recursive-delete ROOT-OR-HOME/ rm --recursive (capital -R is BSD/macOS recursive)
...
# docker recursive-delete ROOT-OR-HOME/ docker system prune
