...
```bash
$GSTACK_ROOT/bin/gstack-skill-end --skill "diagram" --outcome OUTCOME \
...
```bash
BUNDLE=$(python3 "$GSTACK_ROOT/bin/forgecat-asset.py" diagram) || exit 1
...
`<staged>`). If asset verification fails, stop. Obtain consent before running `python3 "$GSTACK_ROOT/bin/forgecat-asset.py" diagram --allow-download`. The first fetch needs network; verified cache reuse is offline. Do not use a CDN or another renderer.
