...
"$_SS" --skill "design-consultation" --model "claude" --parent-pid "$PPID" --brain-health \
...
A `PRODUCT.md` (impeccable's product-context file) already answers the product questions below: treat it as the user's prior answers, confirm them in one line, and do not re-ask. Never open `.hermes/skills/impeccable/**` or any other skill's files; PRODUCT.md and DESIGN.md are the shared surface.
...
9. **Show screenshots to the user.** After copying a screenshot, use the read_file tool on the copied file so the user sees it inline. Prefer `type: "jpeg", quality: 60` to keep files small.
...
**SERVER FALLBACK:** Nonzero exit or no readiness marker: show each variant inline using the read_file tool (so the user can see them),
...
Use real token values, no placeholders; omit invented `components` entries. Outside plan mode, after writing DESIGN.md, require `bun --no-env-file run $GSTACK_ROOT/bin/gstack-design-md.ts check DESIGN.md` to print `DESIGN_MD_FORMAT: spec`.
