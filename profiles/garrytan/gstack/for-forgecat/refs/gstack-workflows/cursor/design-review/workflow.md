...
```bash
bun --no-env-file run $GSTACK_BIN/gstack-design-detect.ts probe --host cursor
```
...
```bash
bun --no-env-file run $GSTACK_BIN/gstack-design-detect.ts install --host cursor
```
...
```bash
_DJ=$(mktemp); bun --no-env-file run $GSTACK_BIN/gstack-design-detect.ts scan --changed <base> --format gstack --host cursor > "$_DJ"; echo "DETECT_EXIT_CODE=$?"; echo "DETECT_JSON=$_DJ"
```
...
```bash
_DJ=$(mktemp); bun --no-env-file run $GSTACK_BIN/gstack-design-detect.ts scan --format gstack --host cursor "<REPORT_DIR from Setup>/dom/<RUN_ID>" > "$_DJ"; echo "DETECT_EXIT_CODE=$?"; echo "DETECT_JSON=$_DJ"
```
...
- On any Codex error: proceed with cursor (in-host) subagent output only, tagged `[single-model]`.
- If cursor (in-host) subagent also fails: "Outside voices unavailable — continuing with primary review."

Output headers: `CODEX SAYS (design source audit):` and `CURSOR (IN-HOST) SUBAGENT (design consistency):`.
