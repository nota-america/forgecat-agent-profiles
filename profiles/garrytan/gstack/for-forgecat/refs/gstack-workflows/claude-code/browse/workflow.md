...
```bash
$GSTACK_ROOT/bin/gstack-skill-end --skill "browse" --outcome OUTCOME \
...
> **STOP.** Before using any command or snapshot flag beyond the Browser fallback translation table — the full generated reference for every browse command, its argument shape, and every snapshot flag, Read `$GSTACK_ROOT/browse/sections/command-list.md` and execute it
