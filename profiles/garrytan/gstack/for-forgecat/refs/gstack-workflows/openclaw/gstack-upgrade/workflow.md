...
if mv "$TMP_DIR/gstack" "$INSTALL_DIR"; then
  if (cd "$INSTALL_DIR" && ./setup --host openclaw); then
...
LOCAL_GSTACK=""
if [ -n "$_ROOT" ] && [ -d "$_ROOT/$GSTACK_ROOT" ]; then
  _RESOLVED_LOCAL=$(cd "$_ROOT/$GSTACK_ROOT" && pwd -P)
  _RESOLVED_PRIMARY=$(cd "$INSTALL_DIR" && pwd -P)
  if [ "$_RESOLVED_LOCAL" != "$_RESOLVED_PRIMARY" ]; then
    LOCAL_GSTACK="$_ROOT/$GSTACK_ROOT"
  fi
fi
...
```bash
cd "$_ROOT"
git rm -r --cached $GSTACK_ROOT/ 2>/dev/null || true
if ! grep -qF '$GSTACK_ROOT/' .gitignore 2>/dev/null; then
  echo '$GSTACK_ROOT/' >> .gitignore
fi
rm -rf "$LOCAL_GSTACK"
```
...
if cp -Rf "$INSTALL_DIR" "$LOCAL_GSTACK" && rm -rf "$LOCAL_GSTACK/.git" && (cd "$LOCAL_GSTACK" && ./setup --host openclaw); then
...
```
Only on `LOCAL_SYNC_OK`, tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit `$GSTACK_ROOT/` when you're ready." Otherwise stop and report the recovery outcome; do not continue migrations or announce success.
...
$GSTACK_ROOT/bin/gstack-update-check --force 2>/dev/null || true
```
...
**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit `$GSTACK_ROOT/` when you're ready."
