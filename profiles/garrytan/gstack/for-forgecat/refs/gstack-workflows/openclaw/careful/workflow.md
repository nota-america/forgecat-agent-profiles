...
Call `gstack_control` with `operation: "activate"` and `mode: "careful"` before claiming protection is active. Stop if native setup is unavailable.
...
|---------|---------|------|
| `rm -rf` / `rm -r` / `rm --recursive` | `rm -rf ./example-data` | Recursive delete |
