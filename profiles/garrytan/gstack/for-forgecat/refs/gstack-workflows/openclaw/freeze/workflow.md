...
Call the native `gstack_control` tool with `operation: "activate"`, `mode: "freeze"`, and the resolved absolute `boundary`. Only report protection active after the tool confirms success. If the tool is unavailable, stop and complete native setup; do not substitute a prose warning.
