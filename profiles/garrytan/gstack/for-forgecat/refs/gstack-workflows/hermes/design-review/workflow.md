...
"$_SS" --skill "design-review" --model "claude" --parent-pid "$PPID" --brain-health \
...
```bash
bun --no-env-file run $GSTACK_BIN/gstack-design-detect.ts install --host hermes
```
...
```bash
_DJ=$(mktemp); bun --no-env-file run $GSTACK_BIN/gstack-design-detect.ts scan --format gstack --host hermes "<REPORT_DIR from Setup>/dom/<RUN_ID>" > "$_DJ"; echo "DETECT_EXIT_CODE=$?"; echo "DETECT_JSON=$_DJ"
```
