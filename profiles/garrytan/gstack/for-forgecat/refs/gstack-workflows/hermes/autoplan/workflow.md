...
"$_SS" --skill "autoplan" --model "claude" --parent-pid "$PPID" --brain-health \
...
This is qualitatively different from taste decisions. When hermes (in-host) and Codex both
...
- **`disabled`** — the user turned Codex reviews off (`codex_reviews=disabled`). Skip the Codex passes only; the hermes (in-host) adversarial subagent below STILL runs (it is free and fast). Print: "Codex passes skipped (codex_reviews disabled) — running hermes (in-host) adversarial only."
- **`not_installed`** — Codex CLI absent. Print: "Codex not installed — falling back to a hermes (in-host) subagent (fresh context, but the same harness; model identity is unknown). Install Codex for an actual outside-model read: `npm install -g @openai/codex`." Fall back to the hermes (in-host) subagent path.
- **`under_codex`** — stale artifact selected its own harness. Print: "Codex outside review unavailable: harness mismatch; no outside process started. Missing coverage. Repair: setup --host codex." Skip the outside invocation and follow the workflow's native-review instructions below. Conflicting inherited harness markers are not grounds to guess another provider.
- **`not_authed`** — installed but no credentials. Print: "Codex installed but not authenticated — falling back to a hermes (in-host) subagent (same harness; model identity is unknown). Run `codex login` or set `$CODEX_API_KEY`." Fall back to the hermes (in-host) subagent path.
- **`broken_install`** — the CLI is on PATH but cannot execute (spawn ENOENT, non-executable binary, missing vendor payload). Print: "Codex is installed but its binary cannot run — Codex passes skipped. Reinstall: `npm install -g @openai/codex`." Relay the probe's HINT lines and fall back to the hermes (in-host) subagent path. This state exists because a missing binary used to land in the model probe's fail-open bucket and report `ready`, so every Codex pass was skipped silently (#2742).
- **`model_unusable`** — authed but the account cannot use gstack's selected Codex model (#2477: HTTP 400 on every call). Relay the probe's HINT lines, tell the user the one-line fix (set `GSTACK_CODEX_MODEL=<supported-model>` or pass an explicit `-c model=...` override), and fall back to the hermes (in-host) subagent path. The ~10s round trip is cached for 1h; timeouts fail open to `ready`.
...
- Dual voices: always run BOTH hermes (in-host) subagent AND Codex if available (P6).
  Run hermes (in-host) first, then Codex, sequentially;
...
```
  ...
  **hermes (in-host) CEO subagent** (via delegate_task):
...
(CEO — strategy challenge) and hermes (in-host) SUBAGENT (CEO — strategic independence).
Produce CEO consensus table:

```
CEO DUAL VOICES — CONSENSUS TABLE:
═══════════════════════════════════════════════════════════════
  Dimension                           hermes (in-host)  Codex  Consensus
  ──────────────────────────────────── ─────── ─────── ─────────
  1. Premises valid?                   —       —      —
...
**Phase 1 complete.**
Codex: [completed: N concerns / unavailable / disabled]. hermes (in-host) subagent: [completed: N issues / unavailable].
...
- [ ] CEO dual voices ran (Codex + hermes (in-host) subagent, or noted unavailable)
...
```
  ...
  **hermes (in-host) design subagent** (native tool):
  Claude Code: set Agent `native completion before dependent steps` if its schema exposes it.
  Other hosts: foreground; await completion when supported.
...
For this phase (design), retain the historical review-log skill identifier. Add `"host":"hermes","outside_provider":"codex","outside_status":"completed|unavailable|disabled|skipped","phase":"design"`. Record each attempted pass separately when outcomes differ. Use `source:"codex"` only for completed external CLI output, and `source:"in-host"` for a native pass. Historical `source:"claude"` continues to mean a native Claude subagent. CLI availability or a native fallback does not count as outside completion. Preserve reported modelUsage, including multiple models; unknown model identity stays unknown.
...
2. Step 0.5 (Dual Voices): Present the completed calls above under Codex SAYS (design — UX challenge)
   and hermes (in-host) SUBAGENT (design — independent review).
...
```
  ...
  **hermes (in-host) DX subagent** (native tool):
  Claude Code: set Agent `native completion before dependent steps` if its schema exposes it.
  Other hosts: foreground; await completion when supported.
...
For this phase (dx), retain the historical review-log skill identifier. Add `"host":"hermes","outside_provider":"codex","outside_status":"completed|unavailable|disabled|skipped","phase":"dx"`. Record each attempted pass separately when outcomes differ. Use `source:"codex"` only for completed external CLI output, and `source:"in-host"` for a native pass. Historical `source:"claude"` continues to mean a native Claude subagent. CLI availability or a native fallback does not count as outside completion. Preserve reported modelUsage, including multiple models; unknown model identity stays unknown.
...
2. Step 0.5 (Dual Voices): Present the completed calls above under Codex SAYS
   (DX — developer experience challenge) and hermes (in-host) SUBAGENT (DX — independent review).
...
```
  ...
  **hermes (in-host) eng subagent** (native tool):
  Claude Code: set Agent `native completion before dependent steps` if its schema exposes it.
  Other hosts: foreground; await completion when supported.
...
For this phase (eng), retain the historical review-log skill identifier. Add `"host":"hermes","outside_provider":"codex","outside_status":"completed|unavailable|disabled|skipped","phase":"eng"`. Record each attempted pass separately when outcomes differ. Use `source:"codex"` only for completed external CLI output, and `source:"in-host"` for a native pass. Historical `source:"claude"` continues to mean a native Claude subagent. CLI availability or a native fallback does not count as outside completion. Preserve reported modelUsage, including multiple models; unknown model identity stays unknown.
...
2. Step 0.5 (Dual Voices): Present the completed calls above under Codex SAYS
   (eng — architecture challenge) and hermes (in-host) SUBAGENT (eng — independent review).
...
- [ ] Dual voices ran (Codex + hermes (in-host) subagent, or noted unavailable)
- [ ] CEO consensus table produced
...
### Review Scores
- CEO: [summary]
- CEO Voices: Codex [summary], hermes (in-host) subagent [summary], Consensus [X/6 confirmed]
- Design: [summary or "skipped, no UI scope"]
- Design Voices: Codex [summary], hermes (in-host) subagent [summary], Consensus [X/7 confirmed] (or "skipped")
- Eng: [summary]
- Eng Voices: Codex [summary], hermes (in-host) subagent [summary], Consensus [X/6 confirmed]
- DX: [summary or "skipped, no developer-facing scope"]
- DX Voices: Codex [summary], hermes (in-host) subagent [summary], Consensus [X/6 confirmed] (or "skipped")
...
```bash
$GSTACK_ROOT/bin/gstack-review-log '{"skill":"autoplan-voices","run_id":"AUTOPLAN_RUN_ID","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","host":"hermes","outside_provider":"codex","outside_status":"OUTSIDE_STATUS","phase":"ceo","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'

$GSTACK_ROOT/bin/gstack-review-log '{"skill":"autoplan-voices","run_id":"AUTOPLAN_RUN_ID","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","host":"hermes","outside_provider":"codex","outside_status":"OUTSIDE_STATUS","phase":"eng","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
...
```bash
$GSTACK_ROOT/bin/gstack-review-log '{"skill":"autoplan-voices","run_id":"AUTOPLAN_RUN_ID","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","host":"hermes","outside_provider":"codex","outside_status":"OUTSIDE_STATUS","phase":"design","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
...
```bash
$GSTACK_ROOT/bin/gstack-review-log '{"skill":"autoplan-voices","run_id":"AUTOPLAN_RUN_ID","timestamp":"'"$TIMESTAMP"'","status":"STATUS","source":"SOURCE","host":"hermes","outside_provider":"codex","outside_status":"OUTSIDE_STATUS","phase":"dx","via":"autoplan","consensus_confirmed":N,"consensus_disagree":N,"commit":"'"$COMMIT"'"}'
```
...
For this phase (autoplan), retain the historical review-log skill identifier. Add `"host":"hermes","outside_provider":"codex","outside_status":"completed|unavailable|disabled|skipped","phase":"autoplan"`. Record each attempted pass separately when outcomes differ. Use `source:"codex"` only for completed external CLI output, and `source:"in-host"` for a native pass. Historical `source:"claude"` continues to mean a native Claude subagent. CLI availability or a native fallback does not count as outside completion. Preserve reported modelUsage, including multiple models; unknown model identity stays unknown.
