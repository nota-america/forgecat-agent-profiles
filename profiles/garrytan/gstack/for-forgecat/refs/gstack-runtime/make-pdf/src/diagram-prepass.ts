...
export function resolveBundlePath(env: NodeJS.ProcessEnv = process.env): string {
  const root = env.GSTACK_ROOT;
  if (!root || !path.isAbsolute(root)) throw new Error("GSTACK_ROOT must be an absolute runtime path");
  const verified = execFileSync("python3", [path.join(root, "bin/forgecat-asset.py"), "diagram"],
    { env, encoding: "utf8", timeout: 30000 }).trim();
  if (!path.isAbsolute(verified)) throw new Error("asset resolver did not return an absolute path");
  if (env.GSTACK_DIAGRAM_BUNDLE && env.GSTACK_DIAGRAM_BUNDLE !== verified)
    throw new Error("GSTACK_DIAGRAM_BUNDLE differs from verified pinned asset");
  return verified;
}
