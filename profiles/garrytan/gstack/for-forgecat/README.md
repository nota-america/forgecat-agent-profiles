...
The upstream README below retains its source context. Its uninstall section is
adapted to the managed removal procedure above.
...
For this ForgeCat installation, remove the owned native integration with the profile helper, restart native sessions, then quarantine its runtime with `clean`. Use ForgeCat to uninstall its tracked files. Do not run the upstream checkout-removal commands against managed files. See the installed runtime contract for exact commands and retained caches.
