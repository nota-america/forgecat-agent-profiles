...
"$_SS" --skill "spec" --model "claude" --parent-pid "$PPID" --brain-health \
...
Write the prompt with the exact redaction-approved spec bytes using the patch tool; never shell-interpolate the raw draft. Keep hard delimiters and this boundary:
