...
"$_SS" --skill "design-shotgun" --model "claude" --parent-pid "$PPID" --brain-health \
...
A `PRODUCT.md` (impeccable's product-context file) answers the job-to-be-done and audience questions: confirm, do not re-ask. Never open `.hermes/skills/impeccable/**`.
...
tool with `a fresh native leaf worker` and `native completion before dependent steps` for each
