...
version: 0.2.0
description: Launch GStack Browser — AI-controlled Chromium with the sidebar
  extension baked in.
triggers:
  - open gstack browser
  - launch chromium
...
$GSTACK_ROOT/bin/gstack-timeline-log '{"skill":"open-gstack-browser","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$GSTACK_ROOT/bin/gstack-question-log '{"skill":"open-gstack-browser","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
_EXT_PATH=""
_ROOT=$(git rev-parse --show-toplevel 2>/dev/null)
[ -n "$_ROOT" ] && [ -f "$GSTACK_ROOT/extension/manifest.json" ] && _EXT_PATH="$GSTACK_ROOT/extension"
[ -z "$_EXT_PATH" ] && [ -f "$GSTACK_ROOT/extension/manifest.json" ] && _EXT_PATH="$GSTACK_ROOT/extension"
