...
In upstream skill prose, `antislop.md` means the core filter. Read the installed `antislop` skill or the bundled source core at `{{ref:antislop-docs}}/antislop.md`. The upstream usage guide is `{{ref:antislop-docs}}/GUIDE.md`. These reference paths are relative to the project root on project agents and absolute on home agents; do not resolve project paths from the current skill directory.
