...
```bash
npx forgecat install @forgecat/everyinc_compound-engineering-plugin
```

## Components

- **skills/** — 26 Compound Engineering skills for brainstorming, planning, debugging, review, PR workflows, testing, and captured learnings.
- **standalone agents** — 0. Specialist reviewer/research behavior is preserved as skill-local reference assets inside the owning skills, matching the current upstream plugin structure.
