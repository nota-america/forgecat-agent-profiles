...
if [ -f "$SKILL_DIR/scripts/check-health.sh" ]; then
  bash "$SKILL_DIR/scripts/check-health.sh" --version VERSION
else
  echo "Bundled health script not found at $SKILL_DIR/scripts/check-health.sh; run the inline checks from ce-setup instead."
fi
```
