...
Xi Xu Agent Skills collection packaged as one forgecat profile.
...
```bash
npx forgecat install @forgecat/xixu-me_skills
```
...
- Runtime dependencies are skill-specific. See each bundled `SKILL.md`, `references/`, and `scripts/` directory for workflow details.
