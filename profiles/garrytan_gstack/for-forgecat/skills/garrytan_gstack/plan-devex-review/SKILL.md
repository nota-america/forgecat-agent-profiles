...
GSTACK_ROOT="${GSTACK_ROOT:-$(find "$_ROOT" -path "*/skills/garrytan_gstack" -type d ! -path "*/.forgecat/*" 2>/dev/null | head -n 1)}"
if [ -z "$GSTACK_ROOT" ]; then
  GSTACK_ROOT="$(find "$HOME" -maxdepth 5 -path "*/skills/garrytan_gstack" -type d 2>/dev/null | head -n 1)"
fi
if [ -z "$GSTACK_ROOT" ]; then
...
$GSTACK_ROOT/bin/gstack-timeline-log '{"skill":"plan-devex-review","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
Before each AskUserQuestion, choose `question_id` from `scripts/question-registry.ts` or `{skill}-{slug}`, then run `$GSTACK_ROOT/bin/gstack-question-preference --check "<id>"`. `AUTO_DECIDE` means choose the recommended option and say "Auto-decided [summary] → [option] (your preference). Change with /plan-tune." `ASK_NORMALLY` means ask.

After answer, log best-effort:
```bash
$GSTACK_ROOT/bin/gstack-question-log '{"skill":"plan-devex-review","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
$GSTACK_ROOT/bin/gstack-question-preference --write '{"question_id":"<id>","preference":"<pref>","source":"inline-user","free_text":"<optional original words>"}'
```
...
10. **Progressive disclosure** — Simple case is production-ready, not a toy. Complex case uses the same API. SwiftUI: \`Button("Save") { save() }10. **Progressive disclosure** — Simple case is production-ready, not a toy. Complex case uses the same API. SwiftUI: \ → full customization, same API.
...
```bash
_CROSS_PROJ=$($GSTACK_ROOT/bin/gstack-config get cross_project_learnings 2>/dev/null || echo "unset")
echo "CROSS_PROJECT: $_CROSS_PROJ"
if [ "$_CROSS_PROJ" = "true" ]; then
  $GSTACK_ROOT/bin/gstack-learnings-search --limit 10 --cross-project 2>/dev/null || true
else
  $GSTACK_ROOT/bin/gstack-learnings-search --limit 10 2>/dev/null || true
fi
```
...
If A: run `$GSTACK_ROOT/bin/gstack-config set cross_project_learnings true`
If B: run `$GSTACK_ROOT/bin/gstack-config set cross_project_learnings false`
...
- Auth failure (stderr contains "auth", "login", "unauthorized"): "Codex auth failed. Run \`codex login- Auth failure (stderr contains "auth", "login", "unauthorized"): "Codex auth failed. Run \ to authenticate."
...
**Source attribution:** If the most recent entry for a skill has a \`"via"**Source attribution:** If the most recent entry for a skill has a \ field, append it to the status label in parentheses. Examples: `plan-eng-review` with `via:"autoplan"` shows as "CLEAR (PLAN via /autoplan)". `review` with `via:"ship"` shows as "CLEAR (DIFF via /ship)". Entries without a `via` field show as "CLEAR (PLAN)" or "CLEAR (DIFF)" as before.
...
**Verdict logic:**
- **CLEARED**: Eng Review has >= 1 entry within 7 days from either \`review- **CLEARED**: Eng Review has >= 1 entry within 7 days from either \ or \`plan-eng-review- **CLEARED**: Eng Review has >= 1 entry within 7 days from either \`review- **CLEARED**: Eng Review has >= 1 entry within 7 days from either \ or \ with status "clean" (or \`skip_eng_review- **CLEARED**: Eng Review has >= 1 entry within 7 days from either \`review- **CLEARED**: Eng Review has >= 1 entry within 7 days from either \ or \`plan-eng-review- **CLEARED**: Eng Review has >= 1 entry within 7 days from either \`review- **CLEARED**: Eng Review has >= 1 entry within 7 days from either \ or \ with status "clean" (or \ is \`true- **CLEARED**: Eng Review has >= 1 entry within 7 days from either \`review- **CLEARED**: Eng Review has >= 1 entry within 7 days from either \ or \`plan-eng-review- **CLEARED**: Eng Review has >= 1 entry within 7 days from either \`review- **CLEARED**: Eng Review has >= 1 entry within 7 days from either \ or \ with status "clean" (or \`skip_eng_review- **CLEARED**: Eng Review has >= 1 entry within 7 days from either \`review- **CLEARED**: Eng Review has >= 1 entry within 7 days from either \ or \`plan-eng-review- **CLEARED**: Eng Review has >= 1 entry within 7 days from either \`review- **CLEARED**: Eng Review has >= 1 entry within 7 days from either \ or \ with status "clean" (or \ is \)
- **NOT CLEARED**: Eng Review missing, stale (>7 days), or has open issues
- CEO, Design, and Codex reviews are shown for context but never block shipping
- If \`skip_eng_review- If \ config is \`true- If \`skip_eng_review- If \ config is \, Eng Review shows "SKIPPED (global)" and verdict is CLEARED

**Staleness detection:** After displaying the dashboard, check if any existing reviews may be stale:
- Parse the \`---HEAD---- Parse the \ section from the bash output to get the current HEAD commit hash
- For each review entry that has a \`commit- For each review entry that has a \ field: compare it against the current HEAD. If different, count elapsed commits: \`git rev-list --count STORED_COMMIT..HEAD- For each review entry that has a \`commit- For each review entry that has a \ field: compare it against the current HEAD. If different, count elapsed commits: \. Display: "Note: {skill} review from {date} may be stale — {N} commits since review"
- For entries without a \`commit- For entries without a \ field (legacy entries): display "Note: {skill} review from {date} has no commit tracking — consider re-running for accurate staleness detection"
...
- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \`mode- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \, \`scope_proposed- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \`mode- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \, \, \`scope_accepted- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \`mode- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \, \`scope_proposed- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \`mode- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \, \, \, \`scope_deferred- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \`mode- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \, \`scope_proposed- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \`mode- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \, \, \`scope_accepted- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \`mode- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \, \`scope_proposed- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \`mode- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \, \, \, \, \`commit- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \`mode- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \, \`scope_proposed- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \`mode- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \, \, \`scope_accepted- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \`mode- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \, \`scope_proposed- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \`mode- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \, \, \, \`scope_deferred- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \`mode- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \, \`scope_proposed- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \`mode- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \, \, \`scope_accepted- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \`mode- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \, \`scope_proposed- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \`mode- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \`critical_gaps- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \`unresolved- **plan-ceo-review**: \`status- **plan-ceo-review**: \, \, \, \, \, \, \, \
  → Findings: "{scope_proposed} proposals, {scope_accepted} accepted, {scope_deferred} deferred"
  → If scope fields are 0 or missing (HOLD/REDUCTION mode): "mode: {mode}, {critical_gaps} critical gaps"
- **plan-eng-review**: \`status- **plan-eng-review**: \, \`unresolved- **plan-eng-review**: \`status- **plan-eng-review**: \, \, \`critical_gaps- **plan-eng-review**: \`status- **plan-eng-review**: \, \`unresolved- **plan-eng-review**: \`status- **plan-eng-review**: \, \, \, \`issues_found- **plan-eng-review**: \`status- **plan-eng-review**: \, \`unresolved- **plan-eng-review**: \`status- **plan-eng-review**: \, \, \`critical_gaps- **plan-eng-review**: \`status- **plan-eng-review**: \, \`unresolved- **plan-eng-review**: \`status- **plan-eng-review**: \, \, \, \, \`mode- **plan-eng-review**: \`status- **plan-eng-review**: \, \`unresolved- **plan-eng-review**: \`status- **plan-eng-review**: \, \, \`critical_gaps- **plan-eng-review**: \`status- **plan-eng-review**: \, \`unresolved- **plan-eng-review**: \`status- **plan-eng-review**: \, \, \, \`issues_found- **plan-eng-review**: \`status- **plan-eng-review**: \, \`unresolved- **plan-eng-review**: \`status- **plan-eng-review**: \, \, \`critical_gaps- **plan-eng-review**: \`status- **plan-eng-review**: \, \`unresolved- **plan-eng-review**: \`status- **plan-eng-review**: \, \, \, \, \, \`commit- **plan-eng-review**: \`status- **plan-eng-review**: \, \`unresolved- **plan-eng-review**: \`status- **plan-eng-review**: \, \, \`critical_gaps- **plan-eng-review**: \`status- **plan-eng-review**: \, \`unresolved- **plan-eng-review**: \`status- **plan-eng-review**: \, \, \, \`issues_found- **plan-eng-review**: \`status- **plan-eng-review**: \, \`unresolved- **plan-eng-review**: \`status- **plan-eng-review**: \, \, \`critical_gaps- **plan-eng-review**: \`status- **plan-eng-review**: \, \`unresolved- **plan-eng-review**: \`status- **plan-eng-review**: \, \, \, \, \`mode- **plan-eng-review**: \`status- **plan-eng-review**: \, \`unresolved- **plan-eng-review**: \`status- **plan-eng-review**: \, \, \`critical_gaps- **plan-eng-review**: \`status- **plan-eng-review**: \, \`unresolved- **plan-eng-review**: \`status- **plan-eng-review**: \, \, \, \`issues_found- **plan-eng-review**: \`status- **plan-eng-review**: \, \`unresolved- **plan-eng-review**: \`status- **plan-eng-review**: \, \, \`critical_gaps- **plan-eng-review**: \`status- **plan-eng-review**: \, \`unresolved- **plan-eng-review**: \`status- **plan-eng-review**: \, \, \, \, \, \
  → Findings: "{issues_found} issues, {critical_gaps} critical gaps"
- **plan-design-review**: \`status- **plan-design-review**: \, \`initial_score- **plan-design-review**: \`status- **plan-design-review**: \, \, \`overall_score- **plan-design-review**: \`status- **plan-design-review**: \, \`initial_score- **plan-design-review**: \`status- **plan-design-review**: \, \, \, \`unresolved- **plan-design-review**: \`status- **plan-design-review**: \, \`initial_score- **plan-design-review**: \`status- **plan-design-review**: \, \, \`overall_score- **plan-design-review**: \`status- **plan-design-review**: \, \`initial_score- **plan-design-review**: \`status- **plan-design-review**: \, \, \, \, \`decisions_made- **plan-design-review**: \`status- **plan-design-review**: \, \`initial_score- **plan-design-review**: \`status- **plan-design-review**: \, \, \`overall_score- **plan-design-review**: \`status- **plan-design-review**: \, \`initial_score- **plan-design-review**: \`status- **plan-design-review**: \, \, \, \`unresolved- **plan-design-review**: \`status- **plan-design-review**: \, \`initial_score- **plan-design-review**: \`status- **plan-design-review**: \, \, \`overall_score- **plan-design-review**: \`status- **plan-design-review**: \, \`initial_score- **plan-design-review**: \`status- **plan-design-review**: \, \, \, \, \, \`commit- **plan-design-review**: \`status- **plan-design-review**: \, \`initial_score- **plan-design-review**: \`status- **plan-design-review**: \, \, \`overall_score- **plan-design-review**: \`status- **plan-design-review**: \, \`initial_score- **plan-design-review**: \`status- **plan-design-review**: \, \, \, \`unresolved- **plan-design-review**: \`status- **plan-design-review**: \, \`initial_score- **plan-design-review**: \`status- **plan-design-review**: \, \, \`overall_score- **plan-design-review**: \`status- **plan-design-review**: \, \`initial_score- **plan-design-review**: \`status- **plan-design-review**: \, \, \, \, \`decisions_made- **plan-design-review**: \`status- **plan-design-review**: \, \`initial_score- **plan-design-review**: \`status- **plan-design-review**: \, \, \`overall_score- **plan-design-review**: \`status- **plan-design-review**: \, \`initial_score- **plan-design-review**: \`status- **plan-design-review**: \, \, \, \`unresolved- **plan-design-review**: \`status- **plan-design-review**: \, \`initial_score- **plan-design-review**: \`status- **plan-design-review**: \, \, \`overall_score- **plan-design-review**: \`status- **plan-design-review**: \, \`initial_score- **plan-design-review**: \`status- **plan-design-review**: \, \, \, \, \, \
  → Findings: "score: {initial_score}/10 → {overall_score}/10, {decisions_made} decisions"
- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \`mode- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \, \`persona- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \`mode- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \, \, \`competitive_tier- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \`mode- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \, \`persona- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \`mode- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \, \, \, \`unresolved- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \`mode- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \, \`persona- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \`mode- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \, \, \`competitive_tier- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \`mode- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \, \`persona- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \`mode- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \, \, \, \, \`commit- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \`mode- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \, \`persona- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \`mode- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \, \, \`competitive_tier- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \`mode- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \, \`persona- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \`mode- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \, \, \, \`unresolved- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \`mode- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \, \`persona- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \`mode- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \, \, \`competitive_tier- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \`mode- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \, \`persona- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \`mode- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \`tthw_target- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \`tthw_current- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \`product_type- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \`overall_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \`initial_score- **plan-devex-review**: \`status- **plan-devex-review**: \, \, \, \, \, \, \, \, \, \, \
  → Findings: "score: {initial_score}/10 → {overall_score}/10, TTHW: {tthw_current} → {tthw_target}"
- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \`tthw_measured- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \, \`dimensions_tested- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \`tthw_measured- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \, \, \`dimensions_inferred- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \`tthw_measured- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \, \`dimensions_tested- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \`tthw_measured- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \, \, \, \`boomerang- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \`tthw_measured- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \, \`dimensions_tested- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \`tthw_measured- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \, \, \`dimensions_inferred- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \`tthw_measured- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \, \`dimensions_tested- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \`tthw_measured- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \, \, \, \, \`commit- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \`tthw_measured- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \, \`dimensions_tested- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \`tthw_measured- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \, \, \`dimensions_inferred- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \`tthw_measured- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \, \`dimensions_tested- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \`tthw_measured- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \, \, \, \`boomerang- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \`tthw_measured- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \, \`dimensions_tested- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \`tthw_measured- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \, \, \`dimensions_inferred- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \`tthw_measured- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \, \`dimensions_tested- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \`tthw_measured- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \`product_type- **devex-review**: \`status- **devex-review**: \, \`overall_score- **devex-review**: \`status- **devex-review**: \, \, \, \, \, \, \, \
  → Findings: "score: {overall_score}/10, TTHW: {tthw_measured}, {dimensions_tested} tested/{dimensions_inferred} inferred"
- **codex-review**: \`status- **codex-review**: \, \`gate- **codex-review**: \`status- **codex-review**: \, \, \`findings- **codex-review**: \`status- **codex-review**: \, \`gate- **codex-review**: \`status- **codex-review**: \, \, \, \`findings_fixed- **codex-review**: \`status- **codex-review**: \, \`gate- **codex-review**: \`status- **codex-review**: \, \, \`findings- **codex-review**: \`status- **codex-review**: \, \`gate- **codex-review**: \`status- **codex-review**: \, \, \, \
...
|--------|---------|-----|------|--------|----------|
| CEO Review | \`/plan-ceo-review| CEO Review | \ | Scope & strategy | {runs} | {status} | {findings} |
| Codex Review | \`/codex review| Codex Review | \ | Independent 2nd opinion | {runs} | {status} | {findings} |
| Eng Review | \`/plan-eng-review| Eng Review | \ | Architecture & tests (required) | {runs} | {status} | {findings} |
| Design Review | \`/plan-design-review| Design Review | \ | UI/UX gaps | {runs} | {status} | {findings} |
| DX Review | \`/plan-devex-review| DX Review | \ | Developer experience gaps | {runs} | {status} | {findings} |
\`\\`
...
1. Read the plan file (Read tool) to see its full current content. Search the read
   output for a \`## GSTACK REVIEW REPORT   output for a \ heading anywhere in the file.
2. If found, use the Edit tool to DELETE the entire existing section. Match from
   \`## GSTACK REVIEW REPORT   \ through either the next \`##    \`## GSTACK REVIEW REPORT   \ through either the next \ heading or end of
...
4. Verify with the Read tool that \`## GSTACK REVIEW REPORT4. Verify with the Read tool that \ is the last
...
```bash
$GSTACK_ROOT/bin/gstack-learnings-log '{"skill":"plan-devex-review","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
