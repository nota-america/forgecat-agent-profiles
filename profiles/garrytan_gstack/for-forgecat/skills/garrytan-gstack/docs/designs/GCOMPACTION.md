...
**Target path on approval:** `.claude/skills/garrytan-gstack/docs/designs/GCOMPACTION.md`

This is the preserved design artifact for `gstack compact`. Everything above the first `---` divider below gets extracted verbatim to `.claude/skills/garrytan-gstack/docs/designs/GCOMPACTION.md` on plan approval. Everything after that divider is archived research (office hours + competitive deep-dive + eng-review notes + codex review + research findings) that informed the design.
...
CI enforcement: a `scripts/check-references.ts` script parses `.claude/skills/garrytan-gstack/docs/designs/GCOMPACTION.md` for GitHub URLs and re-runs the license check, failing if any referenced project's license moves off the allowlist.
...
**End of GCOMPACTION.md canonical section.** On plan approval, everything above is copied verbatim to `.claude/skills/garrytan-gstack/docs/designs/GCOMPACTION.md` as a **tabled design artifact**. No code is written; no hook is installed; no CHANGELOG entry is added. The doc exists so a future sprint can unblock quickly when Anthropic ships the built-in-tool output-replace API.
