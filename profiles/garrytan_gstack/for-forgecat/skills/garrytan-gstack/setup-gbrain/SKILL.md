...
$GSTACK_DIR/bin/gstack-timeline-log '{"skill":"setup-gbrain","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$GSTACK_DIR/bin/gstack-question-log '{"skill":"setup-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
$GSTACK_DIR/bin/gstack-gbrain-detect
```
...
**If A (Retry)**: re-run `$GSTACK_DIR/bin/gstack-gbrain-detect`
...
```bash
$GSTACK_DIR/bin/gstack-gbrain-install
```
...
```bash
. $GSTACK_DIR/bin/gstack-gbrain-lib.sh
...
```bash
printf '%s' "$GBRAIN_POOLER_URL" | $GSTACK_DIR/bin/gstack-gbrain-supabase-verify -
```
...
```bash
orgs=$($GSTACK_DIR/bin/gstack-gbrain-supabase-provision list-orgs --json)
```
...
```bash
result=$($GSTACK_DIR/bin/gstack-gbrain-supabase-provision \
  create gbrain "$REGION" "$ORG_SLUG" --json)
INFLIGHT_REF=$(echo "$result" | jq -r .ref)
$GSTACK_DIR/bin/gstack-gbrain-supabase-provision wait "$INFLIGHT_REF" --json
pooler=$($GSTACK_DIR/bin/gstack-gbrain-supabase-provision \
...
verify_json=$(GBRAIN_MCP_TOKEN="$GBRAIN_MCP_TOKEN" \
  $GSTACK_DIR/bin/gstack-gbrain-mcp-verify "$MCP_URL")
...
```bash
$GSTACK_DIR/bin/gstack-gbrain-install || exit $?
...
```bash
current_tier=$($GSTACK_DIR/bin/gstack-gbrain-repo-policy get)
```
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  $GSTACK_DIR/bin/gstack-gbrain-repo-policy set "$REMOTE" "$TIER"
  ```
...
$GSTACK_DIR/bin/gstack-artifacts-init --url-form-supported "$URL_FORM"
$GSTACK_DIR/bin/gstack-config set artifacts_sync_mode artifacts-only
...
try:
    ...
    pass
")
$GSTACK_DIR/bin/gstack-gbrain-source-wireup --strict \
...
```bash
$GSTACK_DIR/bin/gstack-memory-ingest --probe
```
...
After answer:
```bash
$GSTACK_DIR/bin/gstack-config set transcript_ingest_mode <choice>
$GSTACK_DIR/bin/gstack-gbrain-sync --full --no-brain-sync
```
...
```bash
$GSTACK_DIR/bin/gstack-gbrain-detect 2>/dev/null || true
$GSTACK_DIR/bin/gstack-config get transcript_ingest_mode 2>/dev/null || echo "off"
$GSTACK_DIR/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo "off"
