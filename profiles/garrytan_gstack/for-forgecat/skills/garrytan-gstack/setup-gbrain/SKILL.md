...
~/.claude/skills/garrytan-gstack/bin/gstack-timeline-log '{"skill":"setup-gbrain","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
~/.claude/skills/garrytan-gstack/bin/gstack-question-log '{"skill":"setup-gbrain","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
~/.claude/skills/garrytan-gstack/bin/gstack-gbrain-detect
```
...
**If A (Retry)**: re-run `~/.claude/skills/garrytan-gstack/bin/gstack-gbrain-detect`
...
if ! gbrain init --pglite --json $GBRAIN_EMBED_FLAGS; then
  ...
  echo "PGLite directory at ~/.gbrain/pglite/ may be in a partial state — \`remove ~/.gbrain/pglite after confirming the path\` if needed before retrying." >&2
  exit 1
fi
...
```bash
~/.claude/skills/garrytan-gstack/bin/gstack-gbrain-install
```
...
```bash
. ~/.claude/skills/garrytan-gstack/bin/gstack-gbrain-lib.sh
...
```bash
printf '%s' "$GBRAIN_POOLER_URL" | ~/.claude/skills/garrytan-gstack/bin/gstack-gbrain-supabase-verify -
```
...
```bash
orgs=$(~/.claude/skills/garrytan-gstack/bin/gstack-gbrain-supabase-provision list-orgs --json)
```
...
```bash
result=$(~/.claude/skills/garrytan-gstack/bin/gstack-gbrain-supabase-provision \
  create gbrain "$REGION" "$ORG_SLUG" --json)
INFLIGHT_REF=$(echo "$result" | jq -r .ref)
~/.claude/skills/garrytan-gstack/bin/gstack-gbrain-supabase-provision wait "$INFLIGHT_REF" --json
pooler=$(~/.claude/skills/garrytan-gstack/bin/gstack-gbrain-supabase-provision \
...
verify_json=$(GBRAIN_MCP_TOKEN="$GBRAIN_MCP_TOKEN" \
  ~/.claude/skills/garrytan-gstack/bin/gstack-gbrain-mcp-verify "$MCP_URL")
...
```bash
~/.claude/skills/garrytan-gstack/bin/gstack-gbrain-install || exit $?
...
if ! gbrain init --pglite --json $GBRAIN_EMBED_FLAGS; then
  ...
  echo "gbrain init failed. Existing config (if any) was restored. PGLite at ~/.gbrain/pglite/ may be in a partial state — \`remove ~/.gbrain/pglite after confirming the path\` to reset." >&2
...
```bash
current_tier=$(~/.claude/skills/garrytan-gstack/bin/gstack-gbrain-repo-policy get)
```
...
- `unset` → AskUserQuestion: "How should `<normalized-remote>` interact with
  ...
  ```bash
  ~/.claude/skills/garrytan-gstack/bin/gstack-gbrain-repo-policy set "$REMOTE" "$TIER"
  ```
...
~/.claude/skills/garrytan-gstack/bin/gstack-artifacts-init --url-form-supported "$URL_FORM"
~/.claude/skills/garrytan-gstack/bin/gstack-config set artifacts_sync_mode artifacts-only
...
try:
    ...
    pass
")
~/.claude/skills/garrytan-gstack/bin/gstack-gbrain-source-wireup --strict \
...
```bash
~/.claude/skills/garrytan-gstack/bin/gstack-memory-ingest --probe
```
...
After answer:
```bash
~/.claude/skills/garrytan-gstack/bin/gstack-config set transcript_ingest_mode <choice>
~/.claude/skills/garrytan-gstack/bin/gstack-gbrain-sync --full --no-brain-sync
```
...
```bash
~/.claude/skills/garrytan-gstack/bin/gstack-gbrain-detect 2>/dev/null || true
~/.claude/skills/garrytan-gstack/bin/gstack-config get transcript_ingest_mode 2>/dev/null || echo "off"
~/.claude/skills/garrytan-gstack/bin/gstack-config get artifacts_sync_mode 2>/dev/null || echo "off"
...
- **Concurrent-run lock.** At skill start, `mkdir ~/.gstack/.setup-gbrain.lock.d`
  ...
  is running. Wait for it, or remove `~/.gstack/.setup-gbrain.lock.d` after confirming the path if
