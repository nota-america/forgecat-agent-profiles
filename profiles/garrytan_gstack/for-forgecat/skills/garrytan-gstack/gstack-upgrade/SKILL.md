...
LOCAL_GSTACK=""
if [ -n "$_ROOT" ] && [ -d "$GSTACK_ROOT" ]; then
  _RESOLVED_LOCAL=$(cd "$GSTACK_ROOT" && pwd -P)
  _RESOLVED_PRIMARY=$(cd "$INSTALL_DIR" && pwd -P)
  if [ "$_RESOLVED_LOCAL" != "$_RESOLVED_PRIMARY" ]; then
    LOCAL_GSTACK="$GSTACK_ROOT"
  fi
fi
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit `$GSTACK_ROOT/` when you're ready."
