...
[ -z "$_AUTO" ] && _AUTO=$($GSTACK_DIR/bin/gstack-config get auto_upgrade 2>/dev/null || true)
...
LOCAL_GSTACK=""
if [ -n "$_ROOT" ] && [ -d "$GSTACK_DIR" ]; then
  _RESOLVED_LOCAL=$(cd "$GSTACK_DIR" && pwd -P)
  _RESOLVED_PRIMARY=$(cd "$INSTALL_DIR" && pwd -P)
  if [ "$_RESOLVED_LOCAL" != "$_RESOLVED_PRIMARY" ]; then
    LOCAL_GSTACK="$GSTACK_DIR"
  fi
fi
_TEAM_MODE=$($GSTACK_DIR/bin/gstack-config get team_mode 2>/dev/null || echo "false")
...
```bash
cd "$_ROOT"
git rm -r --cached $GSTACK_DIR/ 2>/dev/null || true
if ! grep -qF '$GSTACK_DIR/' .gitignore 2>/dev/null; then
  echo '$GSTACK_DIR/' >> .gitignore
fi
rm -rf "$LOCAL_GSTACK"
```
...
```
Tell user: "Also updated vendored copy at `$LOCAL_GSTACK` — commit `$GSTACK_DIR/` when you're ready."
...
```bash
$GSTACK_DIR/bin/gstack-update-check --force 2>/dev/null || \
$GSTACK_DIR/bin/gstack-update-check --force 2>/dev/null || true
```
...
**If versions differ:** follow the Step 4.5 sync bash block above to update the local copy from the primary. Tell user: "Global v{PRIMARY_VER} is up to date. Updated local vendored copy from v{LOCAL_VER} → v{PRIMARY_VER}. Commit `$GSTACK_DIR/` when you're ready."
