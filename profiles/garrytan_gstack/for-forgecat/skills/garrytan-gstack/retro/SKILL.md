...
$GSTACK_DIR/bin/gstack-timeline-log '{"skill":"retro","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$GSTACK_DIR/bin/gstack-question-log '{"skill":"retro","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
See $GSTACK_DIR/docs/designs/PLAN_TUNING_V1.md §Workstream C.
...
```bash
$GSTACK_DIR/bin/gstack-learnings-log '{"skill":"retro","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
...
```bash
DISCOVER_BIN=""
[ -x $GSTACK_DIR/bin/gstack-global-discover ] && DISCOVER_BIN=$GSTACK_DIR/bin/gstack-global-discover
[ -z "$DISCOVER_BIN" ] && [ -x $GSTACK_DIR/bin/gstack-global-discover ] && DISCOVER_BIN=$GSTACK_DIR/bin/gstack-global-discover
