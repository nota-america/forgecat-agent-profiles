...
export interface HostConfig {
  ...
  // --- Path Configuration ---
  /** Global install path relative to $HOME (e.g., '.config/opencode/skills/garrytan-gstack'). */
  globalRoot: string;
  /** Project-local skill path relative to repo root (e.g., '.opencode/skills/garrytan-gstack'). */
  localSkillRoot: string;
  /** Gitignored directory under repo root for generated docs (e.g., '.opencode'). */
  hostSubdir: string;
  /** Whether preamble generates $GSTACK_DIR env vars (true for non-Claude hosts). */
  usesEnvVars: boolean;
  ...
  /** Optional repo-local sidecar config (e.g., Codex uses .agents/skills/garrytan-gstack). */
  sidecar?: {
    /** Sidecar path relative to repo root (e.g., '.agents/skills/garrytan-gstack'). */
    path: string;
