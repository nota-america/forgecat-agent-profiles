...
export GSTACK_DIR
GSTACK_BIN="$GSTACK_DIR/bin"
GSTACK_BROWSE="$GSTACK_DIR/browse/dist"
GSTACK_DESIGN="$GSTACK_DIR/design/dist"
GSTACK_MAKE_PDF="$GSTACK_DIR/make-pdf/dist"
`;

  return `## Preamble (run first)

\`\`\`bash
${runtimeRoot}_UPD=$($GSTACK_DIR/bin/gstack-update-check 2>/dev/null || true)
...
$GSTACK_DIR/bin/gstack-timeline-log '{"skill":"${ctx.skillName}","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
