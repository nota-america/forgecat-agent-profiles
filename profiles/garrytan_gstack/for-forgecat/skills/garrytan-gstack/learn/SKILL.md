...
$GSTACK_DIR/bin/gstack-timeline-log '{"skill":"learn","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$GSTACK_DIR/bin/gstack-question-log '{"skill":"learn","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
```bash
eval "$($GSTACK_DIR/bin/gstack-slug 2>/dev/null)"
$GSTACK_DIR/bin/gstack-learnings-search --limit 20 2>/dev/null || echo "No learnings yet."
```
...
```bash
eval "$($GSTACK_DIR/bin/gstack-slug 2>/dev/null)"
$GSTACK_DIR/bin/gstack-learnings-search --query "USER_QUERY" --limit 20 2>/dev/null || echo "No matches."
```
...
```bash
eval "$($GSTACK_DIR/bin/gstack-slug 2>/dev/null)"
$GSTACK_DIR/bin/gstack-learnings-search --limit 100 2>/dev/null
```
...
```bash
eval "$($GSTACK_DIR/bin/gstack-slug 2>/dev/null)"
$GSTACK_DIR/bin/gstack-learnings-search --limit 50 2>/dev/null
```
...
```bash
$GSTACK_DIR/bin/gstack-learnings-log '{"skill":"learn","type":"TYPE","key":"KEY","insight":"INSIGHT","confidence":N,"source":"user-stated","files":["FILE1"]}'
```
