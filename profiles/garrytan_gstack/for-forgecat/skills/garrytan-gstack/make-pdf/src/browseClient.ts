 ...
 *   4. ~/.claude/skills/garrytan-gstack/browse/dist/browse[.exe]
...
export function resolveBrowseBin(env: NodeJS.ProcessEnv = process.env): string {
  ...
  const globalPath = path.join(home, ".claude/skills/garrytan-gstack/browse/dist/browse");
  ...
  throw new BrowseClientError(
    ...
    [
      ...
      "  cd ~/.claude/skills/garrytan-gstack && ./setup",
      "",
