...
$GSTACK_DIR/bin/gstack-timeline-log '{"skill":"qa","event":"started","branch":"'"$_BRANCH"'","session":"'"$_SESSION_ID"'"}' 2>/dev/null &
_HAS_ROUTING="no"
...
```bash
$GSTACK_DIR/bin/gstack-question-log '{"skill":"qa","question_id":"<id>","question_summary":"<short>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
if [ "$_CROSS_PROJ" = "true" ]; then
  $GSTACK_DIR/bin/gstack-learnings-search --limit 10 --query "qa testing bug regression flake fixture" --cross-project 2>/dev/null || true
else
  $GSTACK_DIR/bin/gstack-learnings-search --limit 10 --query "qa testing bug regression flake fixture" 2>/dev/null || true
fi
```
...
3. Copy report template from `$GSTACK_DIR/qa/templates/qa-report-template.md` to output dir
...
Then follow the **per-page exploration checklist** (see `$GSTACK_DIR/qa/references/issue-taxonomy.md`):
...
**Write each issue to the report immediately** using the template format from `$GSTACK_DIR/qa/templates/qa-report-template.md`.
...
```bash
$GSTACK_DIR/bin/gstack-learnings-log '{"skill":"qa","type":"TYPE","key":"SHORT_KEY","insight":"DESCRIPTION","confidence":N,"source":"SOURCE","files":["path/to/relevant/file"]}'
```
