...
gstack skills are Markdown files that Claude Code discovers from a `skills/` directory. Normally they live at `$GSTACK_DIR/` (your global install). But when you're developing gstack itself, you want Claude Code to use the skills *in your working tree* — so edits take effect instantly without copying or deploying anything.
...
4. **Symlink your fork into the project where you hit the bug:**
   ...
   ln -sfn /path/to/your/gstack-fork $GSTACK_DIR
   cd $GSTACK_DIR && bun install && bun run build && ./setup
   ```
...
|--------|---------------------------|
| Output directory | `{skill}/SKILL.md` vs `$GSTACK_DIR-{skill}/SKILL.md` |
| Frontmatter | Full (name, description, hooks, version) vs minimal (name + description) |
| Paths | `$GSTACK_DIR` vs `$GSTACK_DIR` |
...
- **Dev mode shadows your global install.** Project-local skills take priority over `$GSTACK_DIR`. `bin/dev-teardown` restores the global one.
...
ln -sfn /path/to/your/gstack-checkout $GSTACK_DIR
```
...
Remove the project-local symlink. Claude Code falls back to `$GSTACK_DIR/`:
...
```bash
cd $GSTACK_DIR && ./setup --no-prefix   # switch to /qa, /ship
cd $GSTACK_DIR && ./setup --prefix       # switch to /gstack-qa, /gstack-ship
```
