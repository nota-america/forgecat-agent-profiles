...
`bin/install.js` is the single source of truth for provider detection and install behavior.

Helpers:
- `bin/lib/settings.js` — reads/writes JSONC `settings.json` safely
- `bin/lib/openclaw.js` — OpenClaw workspace skill + SOUL bootstrap installer

The shell / PowerShell shims should stay tiny wrappers only.

### Testing

- Installer tests: `node --test tests/installer/*.test.mjs`
- Python validation / compression tests: `pytest tests/`
- Repo sanity: `python tests/verify_repo.py`

### Key maintainer rules

- Edit `skills/<name>/SKILL.md` for behavior changes. Never edit synced copies under `plugins/caveman/skills/`.
- Keep `README.md`, `INSTALL.md`, and the actual installer behavior in sync.
- Preserve the exact trigger phrases promised in docs when editing hook matching logic.
- Do not weaken symlink-safety or safe-write protections in hook/state helpers.
