...
This is the Cursor native command form of /create-plugin. Run it only when the user explicitly selects that slash command. In the preserved instructions below, $ARGUMENTS means the text the user supplied with the invocation. If Cursor presents that text separately rather than expanding the marker, use the invocation text as the Initial request. If no arguments were supplied, treat it as absent and follow the source questions. Use only platform-native equivalents of these source-authorized tools for this workflow: Read, Write, Grep, Glob, Bash, TodoWrite, AskUserQuestion, Skill, Task. Do not add network or external-service access unless the preserved source instructions explicitly require it. Preserve the source question order and approval gates. When instructed to launch a named verifier, use that installed platform agent in a separate context; do not replace it with main-thread self-verification. If delegation is unavailable, report the limitation.
