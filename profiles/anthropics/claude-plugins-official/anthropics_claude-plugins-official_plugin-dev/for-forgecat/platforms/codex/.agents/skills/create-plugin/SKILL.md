---
name: create-plugin
description: Guided end-to-end plugin creation workflow with component design, implementation,
  and validation
metadata:
  forgecat-transform: command-to-explicit-skill-v1
  forgecat-source-command: commands/create-plugin.md
  forgecat-source-sha256: sha256-f2c634dfc0b519dbb1795bd012f44f93a94cabfc2cbd08020ccd1816a435dab8
  forgecat-source-allowed-tools:
  - Read
  - Write
  - Grep
  - Glob
...
# Invocation of create-plugin

This is the Codex skill form of /create-plugin. Invoke it explicitly as $create-plugin.
In the preserved instructions below, $ARGUMENTS means the text the user supplied with this invocation. It is not an environment variable or an automatic shell expansion. If no arguments were supplied, treat it as absent and follow the source questions. Use only platform-native equivalents of these source-authorized tools for this workflow: Read, Write, Grep, Glob, Bash, TodoWrite, AskUserQuestion, Skill, Task. Do not add network or external-service access unless the preserved source instructions explicitly require it. Preserve the source question order and approval gates. When instructed to launch a named verifier, use that installed platform agent in a separate context; do not replace it with main-thread self-verification. If delegation is unavailable, report the limitation.

When the source names WebFetch or WebSearch, use the available platform web page reading or search capability for the same task. If unavailable, report it; do not pretend current documentation or package versions were checked.

## Preserved source instructions
